...
const prepared=reviewRequest('Fix the failing tests','All tests passed. The bug is fixed.',{mutations:1,unknownMutations:0,dropped:0,checks:[{id:'t',kind:'test',status:'failed',afterMutation:1,call:'bun run test'}],observations:[{id:'t',tool:'bash',call:'bun run test',output:'Test suite failed: expected 2, received 3. Exit code 1.',status:'error',mutation:false,sequence:1}]});
